...
local Theme = {
    MainBG = Color3.fromRGB(7, 9, 15),
    PanelBG = Color3.fromRGB(13, 16, 25),
    CodeBG = Color3.fromRGB(9, 11, 18),
    TopBar = Color3.fromRGB(11, 14, 22),
    Accent = Color3.fromRGB(88, 166, 255),       -- Cyan Blue
    AccentPurple = Color3.fromRGB(167, 139, 250), -- Violet (RemoteFunction)
    Danger = Color3.fromRGB(235, 75, 85),         -- Close button hover
    ButtonBG = Color3.fromRGB(19, 23, 35),
    ButtonHover = Color3.fromRGB(29, 36, 54),
    ButtonActive = Color3.fromRGB(18, 32, 52),    -- Selected / toggled
    TextDark = Color3.fromRGB(146, 154, 176),
    TextLight = Color3.fromRGB(236, 240, 248),
...
-- Gradient accent underline ใต้ TopBar
local AccentLine = Create("Frame",{Parent = Background,BackgroundColor3 = Theme.Accent,BackgroundTransparency = 0.25,BorderSizePixel = 0,Position = UDim2.new(0, 0, 0, 18),Size = UDim2.new(0, 450, 0, 1)})
Create("UIGradient",{Color = ColorSequence.new{ColorSequenceKeypoint(0, Theme.Accent), ColorSequenceKeypoint(1, Theme.AccentPurple)},Parent = AccentLine})
...
Create("UIStroke", {Color = Theme.Accent, Thickness = 1, Transparency = 0.65, Parent = LeftPanel})
Create("TextLabel",{Parent = LeftPanel,BackgroundTransparency = 1,Position = UDim2.new(0, 10, 0, 3),Size = UDim2.new(1, -20, 0, 14),Font = Enum.Font.GothamBold,Text = "REMOTES",TextColor3 = Theme.TextDark,TextSize = 9,TextXAlignment = Enum.TextXAlignment.Left})

local LogList = Create("ScrollingFrame",{Parent = LeftPanel,Active = true,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,BorderSizePixel = 0,Position = UDim2.new(0, 0, 0, 18),Size = UDim2.new(0, 131, 0, 223),CanvasSize = UDim2.new(0, 0, 0, 0),ScrollBarThickness = 2, ScrollBarImageColor3 = Theme.Accent})
local UIListLayout = Create("UIListLayout",{Parent = LogList,HorizontalAlignment = Enum.HorizontalAlignment.Center,SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 3)})

-- ======= Right Panel (Code + Functions) =======
...
Create("UIStroke", {Color = Theme.Accent, Thickness = 1, Transparency = 0.65, Parent = RightPanel})
...
Create("UIStroke", {Color = Theme.Accent, Thickness = 1, Transparency = 0.8, Parent = CodeBox})
...
Create("UIStroke", {Color = Theme.Accent, Thickness = 1, Transparency = 0.65, Parent = TopBar})

local TitleDot = Create("Frame",{Parent = TopBar,BackgroundColor3 = Theme.Accent,BorderSizePixel = 0,Position = UDim2.new(0, 9, 0, 6),Size = UDim2.new(0, 7, 0, 7)})
Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = TitleDot})

local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 22, 0, 0),Size = UDim2.new(0, 57, 0, 18),Font = Enum.Font.GothamBold,Text =  "SimpleSpy",TextColor3 = Theme.TextLight,TextSize = 12,TextXAlignment = Enum.TextXAlignment.Left})

local CloseButton = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Theme.TopBar,BackgroundTransparency = 0, BorderSizePixel = 0,Position = UDim2.new(1, -19, 0, 0),Size = UDim2.new(0, 19, 0, 19),Font = Enum.Font.SourceSans,Text = "",TextColor3 = Color3.new(0, 0, 0),TextSize = 14})
...
local MaximizeButton = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Theme.TopBar,BackgroundTransparency = 0, BorderSizePixel = 0,Position = UDim2.new(1, -38, 0, 0),Size = UDim2.new(0, 19, 0, 19),Font = Enum.Font.SourceSans,Text = "",TextColor3 = Color3.new(0, 0, 0),TextSize = 14})
Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = MaximizeButton})
...
local MinimizeButton = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Theme.TopBar,BackgroundTransparency = 0, BorderSizePixel = 0,Position = UDim2.new(1, -57, 0, 0),Size = UDim2.new(0, 19, 0, 19),Font = Enum.Font.SourceSans,Text = "",TextColor3 = Color3.new(0, 0, 0),TextSize = 14})
...
-- ======= ToolTip =======
local ToolTip = Create("Frame",{Parent = SimpleSpy3,BackgroundColor3 = Theme.TopBar,BackgroundTransparency = 0.03,BorderSizePixel = 0,Size = UDim2.new(0, 200, 0, 50),ZIndex = 5,Visible = false})
Create("UICorner", {CornerRadius = Theme.CornerRadius, Parent = ToolTip})
Create("UIStroke", {Color = Theme.Accent, Thickness = 1, Transparency = 0.4, Parent = ToolTip})
local TextLabel = Create("TextLabel",{Parent = ToolTip,BackgroundTransparency = 1,Position = UDim2.new(0, 8, 0, 6),Size = UDim2.new(0, 184, 0, 38),ZIndex = 5,Font = Enum.Font.GothamMedium,Text = "",TextColor3 = Theme.TextLight,TextSize = 12,TextWrapped = true,TextXAlignment = Enum.TextXAlignment.Left,TextYAlignment = Enum.TextYAlignment.Top})

-------------------------------------------------------------------------------

local selectedColor = Theme.Accent
local deselectedColor = Theme.TextDark
...
function onToggleButtonHover()
    if not toggle then
        TweenService:Create(Simple, TweenInfo.new(0.3), {TextColor3 = Theme.Accent}):Play()
    else
...
function onToggleButtonUnhover()
    TweenService:Create(Simple, TweenInfo.new(0.3), {TextColor3 = Theme.TextLight}):Play()
end
...
function onXButtonUnhover()
    TweenService:Create(CloseButton, TweenInfo.new(0.2), {BackgroundColor3 = Theme.TopBar}):Play()
    end
...
function eventSelect(frame)
    if selected and selected.Log  then
        if selected.Button then
            spawn(function()
                TweenService:Create(selected.Button, TweenInfo.new(0.3), {BackgroundColor3 = Theme.ButtonBG}):Play()
            end)
        end
        selected = nil
    end
    ...
    if selected and selected.Log then
        spawn(function()
            -- สีแดงเข้มเวลาถูกเลือก
            TweenService:Create(frame.Button, TweenInfo.new(0.3), {BackgroundColor3 = Theme.ButtonActive}):Play()
        end)
...
function newButton(name, description, onClick)
    ...
    local Button = Create("TextButton",{Name = "Button",Parent = FunctionTemplate,BackgroundColor3 = Theme.ButtonBG,BorderSizePixel = 0,Position = UDim2.new(0, 2, 0, 2),Size = UDim2.new(0, 90, 0, 23),AutoButtonColor = false,Font = Enum.Font.SourceSans,Text = "",TextColor3 = Color3.new(0, 0, 0),TextSize = 14})
    Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = Button})
    local stroke = Create("UIStroke", {Color = Theme.Accent, Thickness = 1, Transparency = 0.8, Parent = Button})
    
    local ColorBar = Create("Frame",{Name = "ColorBar",Parent = FunctionTemplate,BackgroundColor3 = Theme.Accent,BorderSizePixel = 0,Position = UDim2.new(0, 8, 0, 8),Size = UDim2.new(0, 3, 0, 11),ZIndex = 3})
    ...
    local clickedColor = Theme.ButtonActive
...
function newRemote(type, data)
    ...
    local accentColor = (type == "event" and Theme.Accent) or Theme.AccentPurple

    local RemoteTemplate = Create("Frame",{LayoutOrder = layoutOrderNum,Name = "RemoteTemplate",Parent = LogList,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,Size = UDim2.new(0, 117, 0, 30)})
    
    local Button = Create("TextButton",{Name = "Button",Parent = RemoteTemplate,BackgroundColor3 = Theme.ButtonBG,BorderSizePixel = 0,Position = UDim2.new(0, 4, 0, 2),Size = UDim2.new(0, 115, 0, 26),AutoButtonColor = false,Font = Enum.Font.SourceSans,Text = "",TextColor3 = Color3.new(0, 0, 0),TextSize = 14})
    Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = Button})
    Create("UIStroke", {Color = accentColor, Thickness = 1, Transparency = 0.7, Parent = Button})
...
newButton("คัดลอก Remote", function() return "คัดลอกเส้นทางของ Remote" end, function()
...
newButton("ข้อมูลฟังก์ชัน",function() return "ดูข้อมูลเบื้องหลังฟังก์ชัน" end, function()
    local func = selected and selected.Function
    if func then
        local typeoffunc = typeof(func)
        if typeoffunc ~= 'string' then
            codebox:setRaw("--[[กำลังสร้างข้อมูล โปรดรอสักครู่]]")
            RunService.Heartbeat:Wait()
            ...
            local info = {
                info = getinfo(func),
                constants = lclosure and deepclone(getconstants(func)) or "ไม่มีข้อมูล (เป็น C Closure)",
            ...
            if configs.advancedinfo then
                ...
                info["advancedinfo"] = {
                    ...
                    Protos = lclosure and getprotos(func) or "ไม่มีข้อมูล (เป็น C Closure)"
                }
                if Remote:IsA("RemoteFunction") then
                    info["advancedinfo"]["OnClientInvoke"] = getcallbackmember and (getcallbackmember(Remote,"OnClientInvoke") or "N/A") or "ขาดฟังก์ชัน getcallbackmember"
            ...
            codebox:setRaw("--[[กำลังแปลงข้อมูล โปรดรอสักครู่]]")
            selected.Function = v2v({functionInfo = info})
        end
        codebox:setRaw("-- ข้อมูลฟังก์ชันที่ส่งมาถูกสร้าง"..selected.Function)
        TextLabel.Text = "สร้างข้อมูลสำเร็จ!"
    else
        TextLabel.Text = "ไม่พบฟังก์ชัน!"
    end
end)
...
newButton("เคลียร์ Blacklist", function() return "ล้างรายการที่ถูกซ่อนทั้งหมด" end, function()
    blacklist = {}
...
newButton("เคลียร์ Blocklist", function() return "ยกเลิกการบล็อกทั้งหมด" end, function()
    blocklist = {}
    TextLabel.Text = "ยกเลิกการบล็อกสำเร็จ!"
end)

newButton("ถอดรหัส", function() return "ถอดรหัสสคริปต์ต้นทาง" end,function()
    if decompile then
        if selected and selected.Source then
            ...
            codebox:setRaw(DecompiledScripts[Source] or "-- ไม่พบสคริปต์ต้นทาง")
            TextLabel.Text = "ถอดรหัสสำเร็จ!"
        else
            TextLabel.Text = "ไม่พบสคริปต์ต้นทาง!"
        end
    else
        TextLabel.Text = "Executor ไม่รองรับ Decompile"
    end
end)
...
if configs.supersecretdevtoggle then
    ...
    newButton("Load V2",function() return "โหลดเวอร์ชัน 2" end, function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com"))()
    end)
    local SuperSecretFolder = Create("Folder",{Parent = SimpleSpy3})
    newButton("SUPER SECRET BUTTON",function() return "ไม่ระบุ" end, function()
